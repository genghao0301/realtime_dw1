...
//        //2.4 指定从CK自动重启策略
        ...
        SingleOutputStreamOperator<InOrderDetail> map = invtDS.map(x -> {
            ...
            //添加删除条件
            ArrayList<Tuple2<String,Object>> whereSqls = new ArrayList<>();
            //WH_CODE ,CLIENT_CODE ,SKU_CODE ,PALLET_CODE ,ASN_STATUS ,ASN_CODE ,ASN_LINE_NO ,LOCATION_CODE
            whereSqls.add(new Tuple2<>("WH_CODE",inOrderDetail.getWH_CODE()));
            whereSqls.add(new Tuple2<>("CLIENT_CODE",inOrderDetail.getCLIENT_CODE()));
            whereSqls.add(new Tuple2<>("SKU_CODE",inOrderDetail.getSKU_CODE()));
            whereSqls.add(new Tuple2<>("PALLET_CODE",inOrderDetail.getPALLET_CODE()));
            whereSqls.add(new Tuple2<>("ASN_STATUS",inOrderDetail.getASN_STATUS()));
            whereSqls.add(new Tuple2<>("ASN_CODE",inOrderDetail.getASN_CODE()));
            whereSqls.add(new Tuple2<>("ASN_LINE_NO",inOrderDetail.getASN_LINE_NO()));
            whereSqls.add(new Tuple2<>("LOCATION_CODE",inOrderDetail.getLOCATION_CODE()));
